...
const FILE_EXCEED_MSG = 'The number of files on your team's cloud drive has exceeded the limit (400, 000), stop copying'
...
async function gen_count_body ({ fid, type, update, service_account }) {
  async function update_info () {
    const info = await walk_and_save({ fid, update, service_account }) // This step has stored the fid record in the database
  ...
  if (type === 'all') {
    ...
    if (!info) { // The last statistical process was interrupted
...
async function count ({ fid, update, sort, type, output, not_teamdrive, service_account }) {
  ...
  if (!update) {
    ...
    if (info) {
      console.log('Found local cache data, cache time：', dayjs(info.mtime).format('YYYY-MM-DD HH:mm:ss'))
...
function get_all_by_fid (fid) {
  ...
  function recur (result, subf) {
    ...
    const arr = subf.map(v => {
      ...
      if (!row) return null // If no corresponding FID record is found, it means that the process was interrupted last time or the directory reading was not completed
...
async function walk_and_save ({ fid, not_teamdrive, update, service_account }) {
  ...
  const loop = setInterval(() => {
    ...
    const message = `${now} | Obtained object ${result.length} | Network request in progress${limit.activeCount}/Queue${limit.pendingCount}`
  ...
  not_finished.length ? console.log(' Unread directory ID：', JSON.stringify(not_finished)) : console.log('Read all directories finished')
  clearInterval(loop)
...
async function ls_folder ({ fid, not_teamdrive, service_account }) {
  ...
  const use_sa = (fid !== 'root') && (service_account || !not_teamdrive) // use sa by default without parameters
  ...
  do {
    ...
    if (!data) {
      console.error('Reading the directory is not complete (partial reading), parameters:', params)
...
function real_get_sa_token (el) {
  ...
  // The reason for passing the gtoken is that when an account's traffic is exhausted, it can be filtered accordingly
  ...
  return new Promise((resolve, reject) => {
    gtoken.getToken((err, tokens) => {
      if (err) {
        ...
        el.expires = Date.now() + 1000 * (expires_in - 60 * 5) // 5 minutes in advance is judged to be expired
...
async function create_folder (name, parent, use_sa, limit) {
  ...
  while (retry < RETRY_LIMIT) {
    try {
      ...
      console.log('During retry of creating directory：', name, 'Number of retries：', retry)
    }
  }
...
async function user_choose () {
  const answer = await prompts({
    type: 'select',
    name: 'value',
    message: 'The last copy record was detected. Do you want to continue?',
    choices: [
      { title: 'Continue', description: 'continue from where it left off', value: 'continue' },
      { title: 'Restart', description: 'Ignore existing records, copy again', value: 'restart' },
      { title: 'Exit', description: 'Direct exit', value: 'exit' }
    ],
    initial: 0
  })
  return answer.value
...
async function copy ({ source, target, name, min_size, update, not_teamdrive, service_account, dncnr, is_server }) {
  ...
  if (!target) throw new Error('Target position cannot be empty')

  const record = db.prepare('select id, status from task where source=? and target=?').get(source, target)
  if (record && record.status === 'copying') return console.log('A task with the same source and destination is already running, force to quit')
...
async function real_copy ({ source, target, name, min_size, update, dncnr, not_teamdrive, service_account, is_server }) {
  ...
  if (record) {
    ...
    if (choice === 'exit') {
      return console.log(''Exit program')
      ...
      console.log( 'Number of directories to be copied:：', folders.length)
...
async function copy_files ({ files, mapping, service_account, root, task_id }) {
  console.log('\nStart copying files, total number：', files.length)
  const limit = pLimit(PARALLEL_LIMIT)
  let count = 0
  const loop = setInterval(() => {
    const now = dayjs().format('HH:mm:ss')
    const message = `${now} | Number of files copied ${count} | Network request in progress${limit.activeCount}/queue${limit.pendingCount}`
    print_progress(message)
  }, 1000)
...
async function copy_file (id, parent, use_sa, limit) {
  ...
  while (retry < RETRY_LIMIT) {
    ...
    try {
      ...
      if (message && message.toLowerCase().includes('rate limit')) {
        ...
        if (!SA_TOKENS.length) SA_TOKENS = get_sa_batch()
        console.log('This account triggers the usage limit, the remaining number of service account accounts available：', SA_TOKENS.length)
      }
    }
  }
  if (use_sa && !SA_TOKENS.length) {
    ...
    throw new Error('All SA account traffic has been exhausted')
  } else {
    console.warn('Failed to copy file, file id: ' + id)
  }
}
...
async function create_folders ({ source, old_mapping, folders, root, task_id, service_account }) {
  ...
  console.log('Start copying folders, total number：', folders.length)
  const limit = pLimit(PARALLEL_LIMIT)
  let count = 0
  ...
  const loop = setInterval(() => {
    const now = dayjs().format('HH:mm:ss')
    const message = `${now} | Number of directories created ${count} | Network request in progress ${limit.activeCount}/Queue${limit.pendingCount}`
    print_progress(message)
  }, 1000)
...
async function confirm_dedupe ({ file_number, folder_number }) {
  const answer = await prompts({
    type: 'select',
    name: 'value',
    message: ` Delete files ${file_number} and duplicate empty directories ${folder_number} detected at the same location . Are they deleted? `,
    choices: [
      { title: 'Yes', description: 'Confirm deletion', value: 'yes' },
      { title: 'No', description: 'do not delete first', value: 'no' }
    ],
    initial: 0
  })
  return answer.value
...
// Move the file or folder to the recycle bin, need sa as content manager permission and above
...
// Directly delete files or folders without entering the recycle bin, sa is required for manager permission
async function rm_file ({ fid, service_account }) {
  ...
  while (retry < RETRY_LIMIT) {
    try {
      ...
      retry++
      handle_error(err)
      console.log('Delete retry, retry times', retry)
    }
  }
}
  ...
  await Promise.all(dupes.map(async v => {
    try {
      ...
      if (v.mimeType === FOLDER_TYPE) {
        console.log('Successfully deleted folder', v.name)
        folder_count++
      } else {
